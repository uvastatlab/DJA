...
R code needs to be in "chunks" in an R Markdown Notebook. 
...
Run code chunks by clicking the *Run* button within the chunk or by placing your cursor inside it and pressing *Ctrl+Shift+Enter* (Win/Linux) or *Cmd+Shift+Return* (Mac). 
...
We'll use the following packages in this class.
...
Instead of using theory and formulas, let's introduce linear modeling using simulated data. 
...
10 is the y intercept, 5 is the slope. y is completely determined by x. That is, y = 10 + 5*x.
...
1. 10 + 5*x  _fixed_
2. rnorm(n = 25, mean = 0, sd = 10)  _random_
...
The formula "y ~ x" means we think the model is "y = intercept + slope*x". (Unless we specify otherwise, this assumes we want to estimate an intercept.) This tells `lm()` to take our data and find the best intercept and slope. _Notice this is the correct form of the model!_
...
We could use our model estimates to generate data and see if they look similar to our original data. Feel free to repeatedly run the chunk to see different sets of model simulated data.
...
ggplot(d) +
  ...
  geom_point(mapping = aes(x, y2), color = "red")
```

That looks pretty good! Our model-generated data appears similar to our observed data.

We could also compare smooth density curves of the original and model-generated data. Smooth density curves are basically smooth versions of histograms. If we have a good model, data generated by our model should have a similar distribution to the original data. (Run entire chunk at once.) Feel free to repeatedly run the chunk to see different sets of model simulated data.
...
2. determine if the model is useful, is it "good"
3. use the model to explain relationships, provide advice, and/or make predictions
...
We talk about fitting non-linear effects in the _Non-Linear Effects_ section in the appendix.
...
Let's fit a linear model using FinSqFt, Bedroom and LotSize. The plus (+) sign means "include" in model.
...
$$
\text{Mean TotalValue} = -116,101 + 327.9*\text{FinSqFt} + -27,847*\text{Bedroom} + 4515*\text{LotSize}
$$

Some naive interpretation:

- each additional finished square foot adds about $327 to total value
- each additional Bedroom drops the price by $27,847 (?)
- each additional acre of land adds about $4515 to price

The interpretation assumes _all other variables are held constant_! So adding a Bedroom to a house, without increasing the lot size or finished square feet of the house, is estimated to drop the value of a home. Does this make sense? 
...
Another way to assess model fit is by plotting _fitted_ values versus _residuals_. Residuals are the observed data minus the fitted data. We can get that plot by using the `plot()` function on our model and specifying `which = 1`. (There are six possible plots.)
...
- if a residual is 0, that means the model is perfectly fitting the true value.
- if a residual is positive, that means the observed value is _higher_ than the fitted value. The model under predicts those values.
- if a residual is negative, that means the observed value is _lower_ than the fitted value. The model over predicts those values.
- Ideally, residuals will be uniformly clustered around 0 (not the case in this plot!)

By default, the three most "extreme" values are labeled. The house in row 17240 has a fitted value of almost 6 million, but the residual is almost 8 million. This model way underfits this observation. 

The observed value of this home is over 13 million.

```{r}
homes[17240, "TotalValue"]
```

This plot reveals that our assumption of a constant standard deviation is highly suspect. Non-constant standard deviation can be evidence of a wrong model or a very skewed response (or a bit of both). Recall that our response is quite skewed:
...
The residual vs fitted plot looks somewhat better, though we see we're underfitting some homes. 

```{r}
plot(m2, which = 1)
```

Let's simulate data and compare to observed data.
...
Increasing something by a percentage is a _multiplicative effect_.

- If everyone at a company gets a $100 raise, that's additive.
- If everyone at a company gets a 3% raise, that's multiplicative. We multiply the salary by 1.03 to get the new salary.
...
- Std. Error: measures uncertainty of estimate.
- t value: Estimate/Std. Error (ratio)
- Pr(>|t|): Probability of t value that large or larger if coefficient = 0

- Residual standard error: estimate of the constant standard deviation of the normal dist'n of the residuals (noise)
- degrees of freedom: sample size (31324) - number of coefficients (4)
...
HSDistrict does not contain numbers, it contains names. So how can we add it to a linear model? We use indicator variables. We'll add two predictors to our model: 

- Monticello
- Western Albemarle

We will indicate membership membership in HSDistrict as follows:
...
Fortunately R takes care of this for us!
