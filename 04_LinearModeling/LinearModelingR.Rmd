...
$$
\text{Total Value} = -116,101 + 327.9*\text{FinSqFt} + -27,847*\text{Bedroom} + 4515*\text{LotSize}
$$
...
Another way to assess fit is plotting _fitted_ values versus _residuals_. Residuals are the observed data minus the fitted data. We can get that plot by using the `plot()` function on our model and specifying `which = 1`. (There are six possible plots.)
...
The diagnostic plot looks somewhat better, though we see we're underfitting some homes. 
