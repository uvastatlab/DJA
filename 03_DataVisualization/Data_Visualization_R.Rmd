...
```{r}
ggplot(alb) +
  aes(x = INTPTLON, y = INTPTLAT, label = NAME) +
  geom_sf() +
...
ggplot(alb) +
  aes(fill = TotalValue) +
  geom_sf() +
  geom_text(aes(y = as.numeric(INTPTLAT), x = as.numeric(INTPTLON), label = NAME)) +
  scale_fill_continuous("Median home value", 
                        labels = dollar) +
  theme_void()
ggplotly(tooltip = c("fill", "label"))
