...
That's nice but it would help if the census tracts were labeled. We can do that with `geom_text()`. We need to map the NAME column of the alb data frame to the label aesthetic. Set `x = INTPTLON` and `y = INTPTLAT` to place labels at the geographic center of the census tracts.

```{r}
ggplot(alb) +
  geom_sf() +
  geom_text(aes(x = INTPTLON, y = INTPTLAT, label = NAME),
            size = 3) + # size = 3 makes the labels smaller
  theme_void()
```
